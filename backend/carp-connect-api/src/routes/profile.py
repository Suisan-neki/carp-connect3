...
@profile_bp.route('/profile/<user_id>/achievements', methods=['PUT'])
def update_achievements(user_id):
    """ユーザーの実績・バッジを更新"""
    try:
        if user_id not in achievements_storage:
            return jsonify({
                'success': False,
        ...
        if not isinstance(data.get('achievements'), list):
            return jsonify({
                'success': False,
                'error': 'Invalid data format, achievements should be a list'
            }), 400

        achievements_storage[user_id] = data['achievements']
        
        return jsonify({
            'success': True,
            'achievements': achievements_storage[user_id]
        }), 200
    except Exception as e:
        return jsonify({
            'success': False,
